...
| B-004 | Medium | Repo hygiene | No `.gitignore` at root or in `landing-page/`; no `.env.example` | Risk of future secret/artifact commits once backend+env added | Add `.gitignore` (+ nested) and `.env.example` before backend env work; commit Session 01 | RESOLVED — root `.gitignore` (node_modules/dist/.env/src/generated/) + `.env.example` present & tracked; `landing-page/` is a prototype |
