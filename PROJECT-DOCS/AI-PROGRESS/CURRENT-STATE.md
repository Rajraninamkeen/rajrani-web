...
| Current session | Session 30 — pluggable courier-provider integration (tracking + POD) done & pushed. Session-18 Razorpay pattern: `src/commerce/courier` provider layer (sandbox default + real REST http) selected via `COURIER_PROVIDER`; courier pickup books a provider waybill and deliver captures a POD on slice-parcel + replacement assignments (tracking/POD columns via migration `courier_tracking`). Verified live over the default sandbox AND a real-HTTP provider against a local courier-protocol mock (`scripts/courier-mock.mjs`). Per-slice payout deferred (seller earn stays order-level) |
...
| Repository health | pushed to GitHub (Sessions 28–30; see git log, `origin/main`); no committed secrets |
