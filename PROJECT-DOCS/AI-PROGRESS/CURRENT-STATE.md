...
| Last verification | typecheck/build OK; `npm test` **173 passing (18 suites)**; **21 migrations applied** (`migrate status` up to date); live Session 18 E2E on `:4000` running the **Razorpay provider** against the **local Razorpay-protocol mock** `:3911` (`node scripts/razorpay-mock.mjs`; run `scripts/e2e-razorpay.mjs`) — buy-now PREPAID `BK-MTS15LRA` → razorpay order `order_35lrm` created via mock + stored in `Payment.providerPaymentId` (provider `razorpay`); raw-body-HMAC `payment.captured` webhook → order PAID (idempotent replay; bad signature 403); operator deliver to DELIVERED; customer return → decision/pickup/picked-up/inspection PASS → refund `RFD-MTS15LYS-FME3` initiate + **complete executed a LIVE razorpay refund on the mock** recording real `gatewayRef rfnd_mts15lzekxyz` with `gatewayProvider=razorpay` and a `refund_transactions` SUCCESS row; order → REFUNDED/REFUNDED. **Session 19 async reconciliation** on `:4100` (razorpay) vs `:3912` (mock `REFUND_ASYNC=1`, `scripts/e2e-refund-async.mjs` → 20 ok steps): order `BK-MTS23JIS` → refund `RFD-MTS23K0U-ZB5G` submitted in-flight (Refund **PROCESSING**, `gatewayRef rfnd_mts23k1hwpmh`, `REFUND_PROCESSING` event) → signed `refund.processed` webhook reconciled it to **COMPLETED** (Refund + `refund_transactions` SUCCESS, return request COMPLETED, order REFUNDED/REFUNDED, `REFUND_COMPLETED` SYSTEM event), replay idempotent. Prior Session 14–18 E2E remain valid (2026-09-08). |
| Repository health | pushed to GitHub (`main` up to `34159fb`, Session 18); Session 19 committed locally (code+migration, docs) — push pending next owner token; no committed secrets |
...
- Seller onboarding/KYC + orgs (applications/documents/reviews/status-history +
  ...
  or reassign/retry, product-listing/publishing, reviews, real delivery-courier
