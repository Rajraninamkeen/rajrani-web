...
- Returns now support both a **refund** resolution and a **replacement (exchange)**
  ...
  (money logic kept intact for REFUND requests). Outbound replacement dispatch is
  modelled but not yet API-driven. Item-level partial returns, pickup scheduling,
  per-line inspection, the `return_evidence`/`return_items`/`return_events`/
  `return_inspections`/`replacements`/`refund_transactions` records are modelled
  and working (Sessions 08 + 16).
