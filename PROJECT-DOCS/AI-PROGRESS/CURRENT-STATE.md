...
| Current session | Session 30 (in progress) — pluggable courier-provider integration (Session-18 Razorpay pattern): provider layer (sandbox default + real REST http) + tracking/POD recorded on slice-parcel & replacement courier pickup/deliver. Code increment pushed (`origin/main`); per-slice payout deferred (seller earn stays order-level) |
